...
function App(){
  ...
  useEffect(() => {
    window.API.fetchTemplates().then(setTemplates).catch((e)=> setError(e.message));
  }, []);
  ...
  async function onGenerate(){
    ...
    try{
      const blob = await window.API.generateDoc(selected, fields);
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, Segoe UI, Roboto", padding: 20, maxWidth: 720, margin: "0 auto" }}>
          ...
          {schema.fields.map((f)=> (
            <div key={f.key} style={{ marginBottom: 10 }}>
              <label>
                ...
                <br />
                {(["body","summary","experiences","education"]).includes(f.key) ? (
      ...
      <footer style={{ marginTop: 40, opacity: .7 }}>
        <div>v0.1.2 • No-build(React UMD + Babel) 구동</div>
      </footer>
    </div>
  );
}
